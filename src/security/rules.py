...
def _build_recommendations(checks: Dict[str, Dict[str, Any]]) -> List[str]:
    ...
    if not checks["character_diversity"]["passed"]:
        recommendations.append("Büyük harf, küçük harf, rakam ve özel karakter gruplarını birlikte kullanın.")

    if not checks["only_letters_or_digits"]["passed"]:
        recommendations.append("Parolayı yalnızca harflerden veya yalnızca rakamlardan oluşturmayın.")

    if not recommendations:
        recommendations.append("Parola temel kural tabanlı güvenlik kontrollerini başarıyla karşılıyor.")
